...
void PulseRenderer::setFormat(const Format& format)
{
...
void PulseRenderer::queueFrame(const Frame& frame)
{
...
void PulseRenderer::sinkInputInfoCb(pa_context* pContext, const pa_sink_input_info* pInfo, int eol, void* pData)
{
    ...
    if (pInfo && (pInfo->index == pa_stream_get_index(pRenderer->m_pStream)))
    {
        ...
        {
            int32_t volume = (pa_cvolume_avg(&pInfo->volume) * 100) / PA_VOLUME_NORM;
            pRenderer->VolumeChanged(volume);
        }
        else if (pInfo->mute == 1 && pRenderer->m_VolumeInt != 0)
        {
            pRenderer->VolumeChanged(0);
        }
